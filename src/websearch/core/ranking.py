...
def quality_first_ranking(
    ...
    def prepare_engine_results(results: List[Dict[str, Any]], engine: str) -> List[Dict[str, Any]]:
    ...
    logger.info(f"Candidate pool: DDG={len(ddg_prepared)}, Bing={len(bing_prepared)}, Startpage={len(startpage_prepared)}")
    ...
    logger.info(f"Quality ranking: {len(all_candidates)} candidates → {len(deduped)} unique → {len(final_results)} final")
