...
def enforce_domain_diversity(results: List[Dict[str, Any]], max_per_domain: int = 2) -> List[Dict[str, Any]]:
...
def semantic_deduplication(results: List[Dict[str, Any]], similarity_threshold: float = 0.7) -> List[Dict[str, Any]]:
    ...
    for candidate in results:
        ...
        for existing in deduplicated:
            ...
            snippet_similarity = jaccard_similarity_3gram(candidate_snippet, existing_snippet)
            
            # Consider duplicate if either title or snippet is highly similar
            if title_similarity >= similarity_threshold or snippet_similarity >= similarity_threshold:
                is_duplicate = True
...
class SmartDeduplicator:
    ...
    def __init__(self, similarity_threshold: float = 0.7, max_per_domain: int = 2):
    ...
    def process_results(self, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        results_sorted = sorted(results, key=lambda x: x.get('quality_score', 0), reverse=True)
        
        # Step 3: Apply semantic deduplication
        deduplicated = semantic_deduplication(results_sorted, self.similarity_threshold)
        
        # Step 4: Enforce domain diversity
        final_results = enforce_domain_diversity(deduplicated, self.max_per_domain)
        
        logger.info(f"Smart deduplication complete: {len(results)} → {len(final_results)} results")
        
        return final_results


def compare_deduplication_methods(results: List[Dict[str, Any]], num_results: int) -> Dict[str, Any]:
    ...
    domain_improvement = new_analysis['unique_domains'] - old_analysis['unique_domains']
    snippet_improvement = new_analysis['avg_snippet_length'] - old_analysis['avg_snippet_length']
    ...
    return {
        ...
        'improvements': {
            ...
            'percentage_domain_improvement': (domain_improvement / old_analysis['unique_domains'] * 100) if old_analysis['unique_domains'] > 0 else 0
        }
    }
